...
static int parse_transfer_txn_content(buffer_t* rawTxData, fields_array_t* fields) {
    ...
    if (!buffer_can_read(rawTxData, length)) {
        return E_INVALID_DATA;
    }

    if (txn->recipientAddress[0] == MAINNET_NETWORK_TYPE ||
        txn->recipientAddress[0] == TESTNET_NETWORK_TYPE) {
        BAIL_IF(add_new_field(fields,
                              DHP_STR_RECIPIENT_ADDRESS,
                              STI_ADDRESS,
                              DHP_ADDRESS_LENGTH,
    ...
    const bool is_using_mainnet = (transactionContext.bip32Path[1] & 0x7FFFFFFF) ==
                                  10111;  // checks if the coin_type field of bip32 path is 'symbol'
    const uint64_t mosaic_net_id =
    ...
    if (txn->messageSize == 0) {
        ...
        BAIL_IF(add_new_field(fields,
                              DHP_UINT8_TXN_MESSAGE_TYPE,
                              STI_UINT8,
                              sizeof(uint8_t),
                              msgType));  // Show Message Type

        if (*msgType == DHP_PERSISTENT_DELEGATED_HARVESTING)  // TODO: just do one read in a new
                                                              // buffer_t above, and use that below,
                                                              // instead of doing multiple seeks
        {
            // Show persistent harvesting delegation message
#if defined(TARGET_NANOX) || defined(TARGET_NANOS2)
            BAIL_IF(add_new_field(fields,
                                  DHP_STR_TXN_HARVESTING,
                                  STI_HEX_MESSAGE,
                                  txn->messageSize,
                                  buffer_offset_ptr_and_seek(rawTxData, txn->messageSize)));
#elif defined(TARGET_NANOS)
            BAIL_IF(add_new_field(fields,
                                  DHP_STR_TXN_HARVESTING_1,
                                  STI_HEX_MESSAGE,
                                  MAX_FIELD_LEN / 2 - 1,
                                  buffer_offset_ptr_and_seek(rawTxData, MAX_FIELD_LEN / 2 - 1)));
            BAIL_IF(add_new_field(fields,
                                  DHP_STR_TXN_HARVESTING_2,
                                  STI_HEX_MESSAGE,
                                  MAX_FIELD_LEN / 2 - 1,
                                  buffer_offset_ptr_and_seek(rawTxData, MAX_FIELD_LEN / 2 - 1)));
            BAIL_IF(add_new_field(
                fields,
                DHP_STR_TXN_HARVESTING_3,
                STI_HEX_MESSAGE,
                txn->messageSize - MAX_FIELD_LEN + 2,
                buffer_offset_ptr_and_seek(rawTxData, txn->messageSize - MAX_FIELD_LEN + 2)));
#endif
        } else {
            if (!buffer_seek(rawTxData, 1)) {
                return E_NOT_ENOUGH_DATA;
            }  // Message type
            BAIL_IF(add_new_field(
                fields,
                ...
                STI_MESSAGE,
                txn->messageSize - 1,
                buffer_offset_ptr_and_seek(rawTxData,
                                           txn->messageSize - 1)));  // Show Message in plain text
        }
    }
...
static int parse_multisig_account_modification_txn_content(buffer_t* rawTxData,
    ...
    for (uint8_t i = 0; i < txn->addressAdditionsCount; i++) {
        BAIL_IF(add_new_field(fields,
                              ...
                              STI_ADDRESS,
                              DHP_ADDRESS_LENGTH,
                              buffer_offset_ptr_and_seek(rawTxData, DHP_ADDRESS_LENGTH)));
    }

    // Show address deletions count
    BAIL_IF(add_new_field(fields,
                          DHP_UINT8_MAM_DEL_COUNT,
                          STI_UINT8,
                          sizeof(uint8_t),
    ...
    for (uint8_t i = 0; i < txn->addressDeletionsCount; i++) {
        BAIL_IF(add_new_field(
            ...
            DHP_ADDRESS_LENGTH,
            buffer_offset_ptr_and_seek(rawTxData,
                                       DHP_ADDRESS_LENGTH)));  // Read data and security check
    }

    // Show min approval delta
    BAIL_IF(add_new_field(fields,
                          DHP_INT8_MAM_APPROVAL_DELTA,
                          STI_INT8,
                          sizeof(int8_t),
...
static int parse_namespace_registration_txn_content(buffer_t* rawTxData, fields_array_t* fields) {
    ...
    const uint8_t fieldId =
        ((txn->registrationType == 0) ? DHP_UINT64_DURATION : DHP_UINT64_PARENTID);

    BAIL_IF(add_new_field(fields,
                          DHP_UINT8_NS_REG_TYPE,
                          STI_UINT8,
                          sizeof(uint8_t),
    ...
    BAIL_IF(add_new_field(fields,
                          ...
                          STI_STR,
                          txn->nameSize,
                          namespaceName));  // namespace/sub-namespace name
    BAIL_IF(add_new_field(fields,
                          fieldId,
                          STI_UINT64,
                          sizeof(uint64_t),
...
static int parse_account_metadata_txn_content(buffer_t* rawTxData, fields_array_t* fields) {
    ...
    BAIL_IF(add_new_field(
        fields,
        DHP_STR_METADATA_ADDRESS,
        STI_ADDRESS,
        DHP_ADDRESS_LENGTH,
        (const uint8_t*) &txn->address_data.address));  // Show metadata target address
    BAIL_IF(
        add_new_field(fields,
                      DHP_UINT64_METADATA_KEY,
                      STI_UINT64,
                      sizeof(uint64_t),
                      (const uint8_t*) &txn->address_data.metadataKey));  // Show scope metadata key
    BAIL_IF(add_new_field(
        fields,
        DHP_STR_METADATA_VALUE,
        STI_MESSAGE,
        txn->value_data.valueSize,
        buffer_offset_ptr_and_seek(rawTxData, txn->value_data.valueSize)));  // Show different value
    BAIL_IF(
        add_new_field(fields,
                      DHP_INT16_VALUE_DELTA,
                      STI_INT16,
                      sizeof(uint16_t),
...
/**
 * AccountAddressRestrictionTransaction, AccountMosaicRestrictionTransaction and
...
static int parse_account_restriction_txn_content(buffer_t* rawTxData,
        ...
        switch (restrictionType) {
            ...
            case DHP_UINT8_AM_RESTRICTION:
                BAIL_IF(add_new_field(fields,
                                      ...
                                      STI_UINT64,
                                      sizeof(uint64_t),
                                      buffer_offset_ptr_and_seek(rawTxData, sizeof(uint64_t))));
                break;
            case DHP_UINT8_AO_RESTRICTION:
                BAIL_IF(add_new_field(fields,
                                      DHP_UINT16_ENTITY_RESTRICT_OPERATION,
                                      STI_UINT16,
                                      sizeof(uint16_t),
                                      buffer_offset_ptr_and_seek(rawTxData, sizeof(uint16_t))));
                break;
            default:
                return E_INVALID_DATA;
        }
    ...
    BAIL_IF(add_new_field(fields,
                          DHP_UINT16_AR_RESTRICT_OPERATION,
                          STI_UINT16,
                          sizeof(int16_t),
    ...
    if (restrictionType != DHP_UINT8_AM_RESTRICTION) {
        ...
        BAIL_IF(add_new_field(fields,
                              DHP_UINT16_AR_RESTRICT_DIRECTION,
                              STI_UINT16,
                              sizeof(int16_t),
                              (const uint8_t*) &txn->restrictionFlags));
    }
...
static int parse_inner_txn_content(buffer_t* rawTxData,
    ...
    do {
        ...
        BAIL_IF(add_new_field(
            fields,
            isCosigning ? DHP_UINT16_TRANSACTION_DETAIL_TYPE : DHP_UINT16_INNER_TRANSACTION_TYPE,
            STI_UINT16,
            sizeof(uint16_t),
            (const uint8_t*) &txn->innerTxType));
        switch (txn->innerTxType) {
            case DHP_TXN_TRANSFER: {
                BAIL_IF(parse_transfer_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_MOSAIC_DEFINITION: {
                BAIL_IF(parse_mosaic_definition_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_MOSAIC_SUPPLY_CHANGE: {
                BAIL_IF(parse_mosaic_supply_change_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_MODIFY_MULTISIG_ACCOUNT: {
                BAIL_IF(parse_multisig_account_modification_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_REGISTER_NAMESPACE: {
                BAIL_IF(parse_namespace_registration_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_ACCOUNT_METADATA: {
                BAIL_IF(parse_account_metadata_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_MOSAIC_METADATA: {
                BAIL_IF(parse_mosaic_metadata_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_NAMESPACE_METADATA: {
                BAIL_IF(parse_namespace_metadata_txn_content(rawTxData, fields));
                break;
            }

            case DHP_TXN_ADDRESS_ALIAS: {
                BAIL_IF(parse_address_alias_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_MOSAIC_ALIAS: {
                BAIL_IF(parse_mosaic_alias_txn_content(rawTxData, fields));
                break;
            }

            case DHP_TXN_ACCOUNT_ADDRESS_RESTRICTION: {
                BAIL_IF(parse_account_address_restriction_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_ACCOUNT_MOSAIC_RESTRICTION: {
                BAIL_IF(parse_account_mosaic_restriction_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_ACCOUNT_OPERATION_RESTRICTION: {
                BAIL_IF(parse_account_operation_restriction_txn_content(rawTxData, fields));
                break;
            }

            case DHP_TXN_ACCOUNT_KEY_LINK: {
                BAIL_IF(parse_account_key_link_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_NODE_KEY_LINK: {
                BAIL_IF(parse_node_key_link_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_VRF_KEY_LINK: {
                BAIL_IF(parse_vrf_key_link_txn_content(rawTxData, fields));
                break;
            }

            case DHP_TXN_VOTING_KEY_LINK: {
                BAIL_IF(parse_voting_key_link_txn_content(rawTxData, fields));
                break;
            }
            case DHP_TXN_FUND_LOCK: {
                BAIL_IF(parse_fund_lock_txn_content(rawTxData, fields));
                break;
            }
        ...
        // fill zeros
        bool succ = buffer_seek(
            rawTxData,
            txn->size % ALIGNMENT_BYTES == 0 ? 0 : ALIGNMENT_BYTES - (txn->size % ALIGNMENT_BYTES));
        if (!succ) {
            return E_INVALID_DATA;
        }
...
static int parse_aggregate_txn_content(buffer_t* rawTxData, fields_array_t* fields) {
    ...
    // add fields
    BAIL_IF(add_new_field(fields,
                          DHP_HASH256_AGG_HASH,
                          STI_HASH256,
                          DHP_TRANSACTION_HASH_LENGTH,
                          p_tx_hash));  // add transaction hash
    if (!buffer_can_read(rawTxData, txn->payloadSize)) {
        return E_INVALID_DATA;
    }
...
static int parse_txn_detail(buffer_t* rawTxData,
    ...
    switch (txn->transactionType) {
        case DHP_TXN_TRANSFER: {
            result = parseWithFee(rawTxData, fields, parse_transfer_txn_content);
            break;
        }
        case DHP_TXN_AGGREGATE_COMPLETE: {
            result = parseWithFee(rawTxData, fields, parse_aggregate_txn_content);
            break;
        }
        case DHP_TXN_AGGREGATE_BONDED: {
            result = parseWithFee(rawTxData, fields, parse_aggregate_txn_content);
            break;
        }
        case DHP_TXN_MODIFY_MULTISIG_ACCOUNT: {
            result =
                parseWithFee(rawTxData, fields, parse_multisig_account_modification_txn_content);
            break;
        }
        case DHP_TXN_REGISTER_NAMESPACE: {
            result = parseWithFee(rawTxData, fields, parse_namespace_registration_txn_content);
            break;
        }
        case DHP_TXN_ADDRESS_ALIAS: {
            result = parseWithFee(rawTxData, fields, parse_address_alias_txn_content);
            break;
        }
        case DHP_TXN_MOSAIC_ALIAS: {
            result = parseWithFee(rawTxData, fields, parse_mosaic_alias_txn_content);
            break;
        }

        case DHP_TXN_ACCOUNT_ADDRESS_RESTRICTION: {
            result = parseWithFee(rawTxData, fields, parse_account_address_restriction_txn_content);
            break;
        }
        case DHP_TXN_ACCOUNT_MOSAIC_RESTRICTION: {
            result = parseWithFee(rawTxData, fields, parse_account_mosaic_restriction_txn_content);
            break;
        }
        case DHP_TXN_ACCOUNT_OPERATION_RESTRICTION: {
            result =
                parseWithFee(rawTxData, fields, parse_account_operation_restriction_txn_content);
            break;
        }

        case DHP_TXN_ACCOUNT_KEY_LINK: {
            result = parseWithFee(rawTxData, fields, parse_account_key_link_txn_content);
            break;
        }
        case DHP_TXN_NODE_KEY_LINK: {
            result = parseWithFee(rawTxData, fields, parse_node_key_link_txn_content);
            break;
        }
        case DHP_TXN_VRF_KEY_LINK: {
            result = parseWithFee(rawTxData, fields, parse_vrf_key_link_txn_content);
            break;
        }

        case DHP_TXN_VOTING_KEY_LINK: {
            result = parseWithFee(rawTxData, fields, parse_voting_key_link_txn_content);
            break;
        }
        case DHP_TXN_MOSAIC_DEFINITION: {
            result = parseWithFee(rawTxData, fields, parse_mosaic_definition_txn_content);
            break;
        }
        case DHP_TXN_MOSAIC_SUPPLY_CHANGE: {
            result = parseWithFee(rawTxData, fields, parse_mosaic_supply_change_txn_content);
            break;
        }
        case DHP_TXN_FUND_LOCK: {
            result = parseWithFee(rawTxData, fields, parse_fund_lock_txn_content);
            break;
        }
...
{
    if ((transactionType == DHP_TXN_AGGREGATE_COMPLETE) ||
        (transactionType == DHP_TXN_AGGREGATE_BONDED)) {
        const unsigned char TESTNET_GENERATION_HASH[] = {
            0x49, 0xD6, 0xE1, 0xCE, 0x27, 0x6A, 0x85, 0xB7, 0x0E, 0xAF, 0xE5,
            0x23, 0x49, 0xAA, 0xCC, 0xA3, 0x89, 0x30, 0x2E, 0x7A, 0x97, 0x54,
            0xBC, 0xF1, 0x22, 0x1E, 0x79, 0x49, 0x4F, 0xC6, 0x65, 0xA4};

        const unsigned char MAINNET_GENERATION_HASH[] = {
            0x57, 0xF7, 0xDA, 0x20, 0x50, 0x08, 0x02, 0x6C, 0x77, 0x6C, 0xB6,
            0xAE, 0xD8, 0x43, 0x39, 0x3F, 0x04, 0xCD, 0x45, 0x8E, 0x0A, 0xA2,
            0xD9, 0xF1, 0xD5, 0xF3, 0x1A, 0x40, 0x20, 0x72, 0xB2, 0xD6};
        ...
        const bool hashes_equal =
            memcmp(net_hash, rawTxdata->ptr, DHP_TRANSACTION_HASH_LENGTH) == 0;
